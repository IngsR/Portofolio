...
import { userProfile, engineeringMindsetSkills } from '../data/portfolioData';
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const getSkillIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Binary':
        return <Binary className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />;
      case 'Cpu':
        return <Cpu className="w-4 h-4 text-rose-600 dark:text-rose-400" />;
      default:
        return <Layers className="w-4 h-4 text-slate-800 dark:text-slate-200" />;
    }
  };
    ...
    <div className="space-y-16 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-50/80 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300 text-xs font-semibold">
              ...
              <span>Siap On-Site / WFO di Seluruh Indonesia & Hybrid / Remote</span>
            </div>
            ...
            {/* Bio Narrative */}
            <p className="text-slate-600 dark:text-slate-400 text-base sm:text-lg lg:text-xl leading-relaxed font-normal max-w-4xl">
              Lulusan S1 Teknik Informatika Universitas Putra Indonesia &ldquo;YPTK&rdquo; Padang yang berdedikasi membangun aplikasi web modern berskala penuh dengan <span className="font-semibold text-slate-900 dark:text-white">Next.js, React, TypeScript</span>, perancangan basis data <span className="font-semibold text-slate-900 dark:text-white">PostgreSQL & REST API</span>, pengelolaan lingkungan server <span className="font-semibold text-slate-900 dark:text-white">Linux Debian & Docker</span>, serta pemodelan <span className="font-semibold text-slate-900 dark:text-white">Time Series Forecasting</span>.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 text-xs text-slate-700 dark:text-slate-300">
              <div className="flex items-start gap-2.5 p-3.5 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/5">
                ...
                <div>
                  <span className="font-bold block text-slate-900 dark:text-white">Kesiapan Kerja:</span>
                  <span className="text-slate-600 dark:text-slate-400">On-Site / WFO di Seluruh Indonesia & Remote</span>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2.5 p-3.5 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/5">
                ...
                <div>
                  <span className="font-bold block text-slate-900 dark:text-white">Latar Belakang Pendidikan:</span>
                  <span className="text-slate-600 dark:text-slate-400">S1 Teknik Informatika — UPI &ldquo;YPTK&rdquo;</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 pt-3 border-t border-slate-200 dark:border-white/10 text-xs text-slate-600 dark:text-slate-400">
              <span className="text-slate-400 dark:text-slate-500 font-medium">Tautan Profil:</span>
            ...
            <div className="w-full max-w-sm rounded-3xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-5 sm:p-6 space-y-5 shadow-sm">
              ...
              <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-slate-200 dark:bg-slate-800 border border-slate-200 dark:border-white/10 group">
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (parent && !parent.querySelector('.avatar-fallback')) {
                      ...
                      fallback.className = 'avatar-fallback w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-slate-800 to-slate-950 text-white p-6 text-center';
                      fallback.innerHTML = `
                        ...
                        <p class="text-xs text-slate-400 mt-1">Ganti file di /public/profile.jpg</p>
                      `;
                ...
                <div className="absolute bottom-2 left-2 right-2 bg-slate-950/80 backdrop-blur-md rounded-xl p-2.5 text-white text-[11px] flex items-center justify-between">
                  <div className="flex items-center gap-1.5 truncate">
                    ...
                    <span className="font-semibold truncate">Ikhwan Ramadhan</span>
                  </div>
                  <span className="text-slate-300 text-[10px] shrink-0 font-medium">Full-Stack Dev</span>
                </div>
              </div>
              ...
              <div className="space-y-2.5 text-xs text-slate-600 dark:text-slate-400">
                <div className="flex justify-between py-1 border-b border-slate-200/60 dark:border-white/5">
                  ...
                  <a href={`mailto:${userProfile.email}`} className="font-semibold text-slate-900 dark:text-white hover:underline truncate max-w-[200px]">
                    {userProfile.email}
                  </a>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-200/60 dark:border-white/5">
                  <span className="text-slate-400 font-medium">WhatsApp:</span>
                  <a href={userProfile.whatsappUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
                    {userProfile.phone}
                  </a>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-slate-400 font-medium">Pendidikan:</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-200">S1 Teknik Informatika</span>
                </div>
              </div>
      ...
      <section className="space-y-6">
        <div className="space-y-2 border-b border-slate-200 dark:border-white/10 pb-5">
          ...
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 max-w-3xl leading-relaxed font-normal">
            Saya tidak terlalu menikmati software yang hanya selesai dibuat. Saya lebih tertarik pada proses di baliknya—mencari masalah, mencoba pendekatan, menemukan bagian yang kurang tepat, lalu membuatnya lebih masuk akal.
          </p>
        </div>
