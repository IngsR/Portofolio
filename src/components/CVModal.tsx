        ...
        <div className="p-4 sm:px-6 border-b border-slate-200 dark:border-white/10 bg-slate-50/70 dark:bg-white/[0.02] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[10px] text-slate-500">Pratinjau Dokumen & Siap Cetak PDF</span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 bg-white text-slate-950 space-y-8 font-sans">
              ...
              {experienceData.map((exp) => (
                <div key={exp.id} className="space-y-1.5 text-xs">
                  <div className="flex items-baseline justify-between">
                    <strong className="text-sm font-bold text-slate-950">{exp.role}</strong>
                  ...
                  <div className="flex flex-wrap gap-1 pt-1 text-[11px] text-slate-600">
                    <strong className="text-slate-800">Tech Stack:</strong> {exp.techStack.join(', ')}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              {educationData.map((edu) => (
                <div key={edu.id} className="space-y-1 text-xs">
                  ...
                  <p className="text-slate-600 text-[11px] leading-relaxed">{edu.details}</p>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-2.5 text-xs">
                {certificationsData.map((cert) => (
                  <div key={cert.id} className="space-y-0.5">
                    ...
                    <div className="text-slate-500 text-[11px]">{cert.issuer} • {cert.period || cert.issueDate}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-2 text-xs pt-2">
            ...
            <div className="space-y-1.5 text-slate-700 text-xs">
              <div>
                <strong className="text-slate-950">Full-Stack & Web:</strong> Next.js (App Router, Server Actions), React, TypeScript, JavaScript (ES6+), Tailwind CSS, shadcn/ui, NestJS, Express.
              </div>
              <div>
                <strong className="text-slate-950">Database & SQL:</strong> PostgreSQL, SQL Engineering, Drizzle ORM, Prisma, TypeORM, Turborepo, Auth.js (NextAuth), JWT.
              </div>
              <div>
                <strong className="text-slate-950">DevOps & Server OS:</strong> Docker Containerization, OS Linux (Debian & Ubuntu Server), Git/GitHub Workflow, REST API & OpenAPI.
              </div>
              <div>
                <strong className="text-slate-950">Data Science:</strong> Python (NumPy, Pandas, Scikit-learn), Time Series Forecasting (LSTM), TensorFlow, Streamlit, Folium Geospatial.
              </div>
            </div>
          </div>
