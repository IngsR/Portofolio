    ...
    >
      <div className="space-y-4">
        ...
        <div className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-medium">
          ...
          <span className="font-semibold text-slate-900 dark:text-white">{certificate.issuer}</span>
        </div>
        ...
        {certificate.skills && certificate.skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {certificate.skills.slice(0, 4).map((skill) => (
              <span
                key={skill}
                className="px-2.5 py-0.5 text-xs font-medium rounded-md bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-white/5"
              >
                {skill}
              </span>
            ))}
            {certificate.skills.length > 4 && (
              <span className="px-2 py-0.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
                +{certificate.skills.length - 4}
              </span>
            )}
          </div>
      ...
      >
        <button
          onClick={() => onOpenDetail(certificate)}
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-300 dark:border-white/10 text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10 text-xs font-bold transition-all"
        >
        ...
        {certificate.credentialUrl && (
          <a
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-950 text-white dark:bg-white dark:text-slate-950 text-xs font-bold hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-sm"
          >
