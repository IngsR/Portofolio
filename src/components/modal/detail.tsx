  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
        ...
        >
          {/* Header Bar */}
          <div className="flex items-center justify-between gap-3 p-4 sm:p-6 border-b border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02]">
            <div className="flex items-center gap-2.5 min-w-0">
              <span className="p-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-950 shrink-0">
                {isProject ? (
              ...
              <div className="min-w-0 flex flex-wrap items-center gap-x-2 gap-y-0.5">
                <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-tight">
                  {isProject
                    ? "Detail Proyek Rekayasa"
                    : "Kredensial & Sertifikasi"}
                </span>
                <span className="text-slate-300 dark:text-slate-700 leading-none">
                  •
                </span>
                <span className="text-[11px] sm:text-xs font-semibold text-slate-800 dark:text-slate-200 truncate leading-tight">
                  {itemCategory}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-slate-600 dark:text-slate-300">
                <div className="flex items-center gap-1.5 font-medium min-w-0 max-w-full">
                  ...
                  <span className="truncate">{itemRoleOrIssuer}</span>
                </div>
                <div className="flex items-center gap-1.5 font-medium shrink-0">
                  <Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                  <span className="whitespace-nowrap">{itemPeriod}</span>
                </div>
            ...
            {!isProject && certificate && (
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5 min-w-0">
                    <Award className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0" />
                    <span className="leading-snug">
                      Tangkapan Layar &amp; Bukti Dokumen
                    </span>
                  </span>
                  {certificate.image && (
                    <a
                      ...
                      className="text-[11px] sm:text-xs text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 font-semibold whitespace-nowrap shrink-0"
                    >
                ...
                <div className="w-full relative flex items-center justify-center rounded-xl sm:rounded-2xl overflow-hidden bg-slate-900 border border-slate-200 dark:border-white/10 shadow-md p-3 sm:p-4">
                  <img
                    ...
                    width={1200}
                    height={850}
                    className="max-h-[280px] sm:max-h-[380px] w-auto max-w-full object-contain object-center select-none"
                    referrerPolicy="no-referrer"
                  />
                  {/* Overlay badge: relative di mobile (di bawah gambar), absolute di desktop */}
                  <div className="absolute inset-x-0 bottom-0 p-2.5 sm:p-3 flex flex-wrap items-center justify-center sm:justify-between gap-2 pointer-events-none">
                    <span className="bg-black/75 backdrop-blur-md px-2.5 py-1 rounded-lg font-mono font-bold text-[10px] sm:text-xs max-w-full truncate">
                      {certificate.badgeCode || "VERIFIED CREDENTIAL"}
                    </span>
                    <span className="bg-emerald-600/90 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-semibold flex items-center gap-1 whitespace-nowrap">
                      <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
                      <span className="hidden min-[380px]:inline">
                        Dokumen Terverifikasi
                      </span>
                      <span className="min-[380px]:hidden">Terverifikasi</span>
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 sm:p-6 border-t border-slate-100 dark:border-white/10 bg-slate-50/70 dark:bg-white/[0.02] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3">
            <div className="flex items-center min-w-0">
              {isProject && project && onOpenMarkdown && (
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-slate-300 dark:border-white/15 text-slate-900 dark:text-white hover:bg-slate-200/50 dark:hover:bg-white/10 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
                >
                  <BookOpen className="w-4 h-4 shrink-0" />
                  <span className="truncate">Buka Dokumentasi Penuh (.md)</span>
                </button>
              )}
            </div>

            <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 sm:ml-auto">
              {isProject && project?.githubUrl && (
                <a
                  href={project.githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl border border-slate-300 dark:border-white/15 text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
                >
              ...
              {isProject && project?.demoUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 sm:flex-none min-w-0 px-4 sm:px-5 py-2.5 rounded-xl bg-slate-950 text-white dark:bg-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-100 text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm"
              ...
              {!isProject && certificate?.credentialUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full sm:w-auto min-w-0 px-4 sm:px-5 py-2.5 rounded-xl bg-slate-950 text-white dark:bg-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-100 text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm"
                  title="Buka kredensial terverifikasi"
                >
                  <span className="truncate">
                    Lihat Kredensial / Bukti
                  </span>
                  <ExternalLink className="w-4 h-4 shrink-0" />
                </a>
              )}
